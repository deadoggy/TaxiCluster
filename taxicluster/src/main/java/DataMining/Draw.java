...
public class Draw {
	DataMining.MyCanvas p = new DataMining.MyCanvas();
...
class MyCanvas extends JPanel {
	...
	public void addPoints(Color color , List<Point> points) {
		...
		for(int i = 0 ; i + 1 < points.size() ; i++){
			DataMining.Line l = new DataMining.Line(points.get(i), points.get(i+1));
			list.add(l);
		}
	...
	public void paint(Graphics g) {
		...
		for (Entry<Color , List<Line>> en : lines.entrySet()) {
			...
			for(DataMining.Line l : en.getValue())
