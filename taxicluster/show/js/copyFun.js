		...
		svg.append("g")
			...
			.on("mouseover",function(d,i){
					...
					if(data[d.source.index][d.target.index]!=0)
						d3.select("#textForSimilarity").text("similairty："+new Number(1/data[d.source.index][d.target.index]).toFixed(4));
					else d3.select("#textForSimilarity").text("similarity："+new Number(data[d.source.index][d.target.index]).toFixed(4));
			})
