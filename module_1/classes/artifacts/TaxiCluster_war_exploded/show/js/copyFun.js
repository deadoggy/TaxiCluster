//设置基本参数 
	...
	var DbscanData={rep_order:null,taxi_order:null,eps:null,minpts:null,cluster_type:null,partition_type:null,n:null,m:null,nx:null,ny:null};
	var kMeansData={taxiId:null,comparedTaxiIds:null,index:0,clusterId:null,k:null,iterator:null,partition_type:null,n:null}; 
...
//			  });
    ...
    function showSegment(content,dataOfRe,color){
    	
    	//点击代表轨迹展示对应聚类的线段
    	console.log(dataOfRe.length);
        DbscanData.rep_order = content;
		$.ajax({ 
		      type:"POST", 
				...
				//画出代表轨迹，这里画出被选中的轨迹
					...
					for(var c in repreTrack.points){
						var myLatlng = new google.maps.LatLng(repreTrack.points[c].x,repreTrack.points[c].y);
					 	...
					 	marker = new google.maps.Marker({
					 	  ...
					 	  marker.addListener('click',function(){//点击之后触发的事件
								// alert(this.title);
								 ...
								 showSegment(this.title,data,color);
							 });
							repMarkers.push(marker);
							var latlng = new google.maps.LatLng(repreTrack.points[c].x,repreTrack.points[c].y+0.02);
						 	...
						 	marker = new google.maps.Marker({
							  ...
							  marker.addListener('click',function(){//点击之后触发的事件
								    ...
								    // clickedRep.push(poly);
									 ...
									 showSegment(this.title,data,color_poly);
...
//	$("#all_click").click(function(){
	...
	$("#kmeans_submit").click(function(){
		 ...
		 if(voronoi_submit.style.display=='none'){
			 voronoi_submit.style.display="block";
		  }
